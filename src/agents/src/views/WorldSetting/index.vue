...
import WorldCreatorLayout from './WorldCreatorLayout.vue'
import Step1Essence from './Step1Essence.vue'
import Step2Essence from './Step2Essence.vue'
import Step3Essence from './Step3Essence.vue'
...
// 新增：页面加载时自动读取localStorage
onMounted(() => {
    const data = localStorage.getItem('worldEditData')
    if (data) {
        try {
            worldEditData.value = JSON.parse(data)
            started.value = true
            currentStep.value = 1
        } catch (e) {
            worldEditData.value = null
        }
    }
})

// 提供给子组件
provide('worldEditData', worldEditData)
</script>
